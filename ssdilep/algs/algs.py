...
class CutAlg(pyframe.core.Algorithm):
    ...
    #__________________________________________________________________________
    def cut_MuPairsDeltaPt150(self):
      cname = "MuPairsDeltaPt150"
      pairs = self.store['mu_pairs']
      for p in pairs:
        p.StoreCut(cname,True)
        if abs(p.lead.tlv.Pt() - p.sublead.tlv.Pt())>150*GeV: 
          p.StoreCut(cname,False)
      return True
    ...
    #__________________________________________________________________________
    def cut_TagAndLooseProbe(self):
      muons = self.store['muons']
      lead_mu = muons[0] 
      sublead_mu = muons[1] 
      
      trig = {"HLT_mu20_iloose_L1MU15":0, "HLT_mu50":1}
      lead_mu_trig_match = bool(lead_mu.isTrigMatchedToChain.at(trig["HLT_mu20_iloose_L1MU15"]) or lead_mu.isTrigMatchedToChain.at(trig["HLT_mu50"]))
      lead_mu_is_tight = bool(lead_mu.isIsolated_FixedCutTightTrackOnly and lead_mu.trkd0sig<3.) 
      #lead_mu_is_loose = bool(not lead_mu.isIsolated_FixedCutTightTrackOnly and lead_mu.trkd0sig<10.)

      sublead_mu_trig_match = bool(sublead_mu.isTrigMatchedToChain.at(trig["HLT_mu20_iloose_L1MU15"]) or sublead_mu.isTrigMatchedToChain.at(trig["HLT_mu50"]))
      sublead_mu_is_tight = bool(sublead_mu.isIsolated_FixedCutTightTrackOnly and sublead_mu.trkd0sig<3.)
      #sublead_mu_is_loose = bool(not sublead_mu.isIsolated_FixedCutTightTrackOnly and sublead_mu.trkd0sig<10.)
     
      # tag selection
      if lead_mu_trig_match and lead_mu_is_tight:
        return True
      elif sublead_mu_trig_match and sublead_mu_is_tight: 
        return True
      else: return False
    
    #__________________________________________________________________________
    def cut_TagAndProbeExist(self):
      return bool(self.store['tag'] and self.store['probe'])

    
    #__________________________________________________________________________
    def cut_ProbeIsTightSigTag(self):
      tag = self.store['tag']
      probe = self.store['probe']
      
      trig = {"HLT_mu20_iloose_L1MU15":0, "HLT_mu50":1}
      tag_is_trig_matched = bool(tag.isTrigMatchedToChain.at(trig["HLT_mu20_iloose_L1MU15"]) or tag.isTrigMatchedToChain.at(trig["HLT_mu50"]))
  
      if tag_is_trig_matched:
        if (probe.isIsolated_FixedCutTightTrackOnly and probe.trkd0sig<3.) : return True 
        else: return False
      else: return False

    #__________________________________________________________________________
    def cut_ProbeIsTightFFTag(self):
      tag = self.store['tag']
      probe = self.store['probe']
      
      trig = {"HLT_mu20_L1MU15":0}
      tag_is_trig_matched = bool(tag.isTrigMatchedToChain.at(trig["HLT_mu20_L1MU15"]))
    ...
    #__________________________________________________________________________
    def cut_ProbeIsTightSigTrigMatched(self):
      tag = self.store['tag']
      probe = self.store['probe']
      ...
      if tag_is_trig_matched:
        if (probe.isIsolated_FixedCutTightTrackOnly and probe.trkd0sig<3. and (probe.isTrigMatchedToChain.at(trig["HLT_mu20_iloose_L1MU15"]) or probe.isTrigMatchedToChain.at(trig["HLT_mu50"]))) : return True 
        else: return False
      else: return False

    #__________________________________________________________________________
    def cut_ProbeIsTightFFTrigMatched(self):
      tag = self.store['tag']
      probe = self.store['probe']
      ...
      if tag_is_trig_matched:
        if (probe.isIsolated_FixedCutTightTrackOnly and probe.trkd0sig<3. and (probe.isTrigMatchedToChain.at(trig["HLT_mu20_L1MU15"]))) : return True 
        else: return False
      else: return False

    #__________________________________________________________________________
    def cut_ProbeIsLooseSigTag(self):
      tag = self.store['tag']
      probe = self.store['probe']
      ...
      if tag_is_trig_matched:
        if (not probe.isIsolated_FixedCutTightTrackOnly and probe.trkd0sig<10.) : return True 
        else: return False
      else: return False

    #__________________________________________________________________________
    def cut_ProbeIsLooseFFTag(self):
      tag = self.store['tag']
      probe = self.store['probe']
    ...
    #__________________________________________________________________________
    def cut_ProbeIsLooseSigTrigMatched(self):
      tag = self.store['tag']
      probe = self.store['probe']
      ...
      if tag_is_trig_matched:
        if (not probe.isIsolated_FixedCutTightTrackOnly and probe.trkd0sig<10. and (probe.isTrigMatchedToChain.at(trig["HLT_mu20_iloose_L1MU15"]) or probe.isTrigMatchedToChain.at(trig["HLT_mu50"]))) : return True 
        else: return False
      else: return False
    
    #__________________________________________________________________________
    def cut_ProbeIsLooseFFTrigMatched(self):
      tag = self.store['tag']
      probe = self.store['probe']
      ...
      if tag_is_trig_matched:
        if (not probe.isIsolated_FixedCutTightTrackOnly and probe.trkd0sig<10. and (probe.isTrigMatchedToChain.at(trig["HLT_mu20_L1MU15"]))) : return True 
        else: return False
      else: return False
    ...
    def cut_MuPairsAngleHi10Low25(self):
      ...
      for p in pairs:
        p.StoreCut(cname,False)
        if p.angle>1.0 and p.angle<2.5:
          p.StoreCut(cname,True)
      return True
...
class PlotAlg(pyframe.algs.CutFlowAlg,CutAlg):
    ...
    def plot(self, region, passed, list_cuts, cut, list_weights=None, weight=1.0):
        ...
        # self.h_tag_pt = self.hist('h_tag_pt', "ROOT.TH1F('$', ';p_{T}(tag) [GeV];Events / (1 GeV)', 2000, 0.0, 2000.0)", dir=MUONS)
        # self.h_tag_eta = self.hist('h_tag_eta', "ROOT.TH1F('$', ';#eta(tag);Events / (0.1)', 50, -2.5, 2.5)", dir=MUONS)
        # self.h_tag_phi = self.hist('h_tag_phi', "ROOT.TH1F('$', ';#phi(tag);Events / (0.1)', 64, -3.2, 3.2)", dir=MUONS)
        # self.h_probe_pt = self.hist('h_probe_pt', "ROOT.TH1F('$', ';p_{T}(probe) [GeV];Events / (1 GeV)', 2000, 0.0, 2000.0)", dir=MUONS)
        # self.h_probe_eta = self.hist('h_probe_eta', "ROOT.TH1F('$', ';#eta(probe);Events / (0.1)', 50, -2.5, 2.5)", dir=MUONS)
        # self.h_probe_phi = self.hist('h_probe_phi', "ROOT.TH1F('$', ';#phi(probe);Events / (0.1)', 64, -3.2, 3.2)", dir=MUONS)
        ...
        # self.h_mumu_deltapt = self.hist('h_mumu_deltapt', "ROOT.TH1F('$', ';#Delta p_{T}(#mu#mu) [GeV];Events / (1 GeV)',2000,0.0,2000.0)", dir=PAIRS)
        # self.h_mumu_reldeltapt = self.hist('h_mumu_reldeltapt', "ROOT.TH1F('$', ';Relative #Delta p_{T}(#mu#mu);Events',100,0.0,1.0)", dir=PAIRS)
        ...
        if passed:
         ...
         # self.h_probe_pt.Fill(probe.tlv.Pt()/GeV, weight)
         # self.h_probe_eta.Fill(probe.tlv.Eta(), weight)
         # self.h_probe_phi.Fill(probe.tlv.Phi(), weight)
         # self.h_tag_pt.Fill(tag.tlv.Pt()/GeV, weight)
         # self.h_tag_eta.Fill(tag.tlv.Eta(), weight)
         # self.h_tag_phi.Fill(tag.tlv.Phi(), weight)
         ...
         """  
          ...
          for mp in mupairs:
            ...
            if pcut:           
             ...
             self.h_mumu_deltapt.Fill(mp.deltapt/GeV, pweight * weight)
             self.h_mumu_reldeltapt.Fill(mp.reldeltapt, pweight * weight)
